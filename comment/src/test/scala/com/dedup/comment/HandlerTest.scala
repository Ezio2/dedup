...
import com.dedup.util.{IdGenerator, Similarity}
...
class HandlerTest extends FunSuite {
  ...
  val jt = conf.getDouble("threshold.jaccard.t")
  val lf = conf.getDouble("threshold.levenshtein.filter_jaccard")
  val lt = conf.getDouble("threshold.levenshtein.t")
  val handler = new Handler(forwardDir.toString, 50000, 3600 * 1000,
    invertDir.toString, 50000, 3600 * 1000, idGenerator, jt, lf, lt)
  ...
  def distance(a: String, b: String) = {
    ...
    if (jSimilarity >= lf) info(s"jSimilar:$jSimilarity lSimilar:$lSimilarity id:${ca.id} vs id:${cb.id} ")
  }
